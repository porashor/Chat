    ...
    <div className='w-full lg:w-[68%] bg-slate-800 rounded-xl py-3 md:py-10 px-2 text-white md:px-5 min-h-screen h-screen overflow-hidden'>
      ...
      <div className='w-full h-full flex flex-col items-center justify-between'>
        ...
        <div className='w-full h-full overflow-y-auto flex flex-col'>
          {
            msgloading ? <div className='w-full h-full flex items-center justify-center animate-spin'><AiOutlineLoading3Quarters/></div> :( Array.isArray(message) && message.map((item, index) =>(
        ...
        <div className='flex items-center justify-between w-full gap-2 md:gap-5'>
            ...
            <button onClick={()=>{sendmessage({person :person._id, me: me._id, message: text}); setText('');messageget({person: person._id, me: me._id})}} className='w-[10%] py-2 bg-[#1A535C] text-white rounded-md px-5 flex items-center justify-center'>
